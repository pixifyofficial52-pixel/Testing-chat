...
async function getDeviceInfo() {
    try {
        ...
        console.log('Using fallback device ID generation');
...
async function generateUniqueUserId() {
    try {
        ...
        console.log('Error generating ID, using fallback');
        const fallbackId = 'usr_' + Date.now() + '_' + Math.random().toString(36).substring(2, 10);
        localStorage.setItem('hj-permanent-user-id', fallbackId);
        return fallbackId;
    }
}
...
function login() {
    ...
    if (!name || !id) { 
        showToast('Please enter both name and ID', 'warning'); 
        return;
    }
...
function completeLogin() {
    console.log('Complete login called');
    const name = document.getElementById('login-name').value.trim();
    const id = document.getElementById('login-userid').value.trim();
...
window.addEventListener('load', async () => {
    ...
    try {
        ...
        // Fallback ID
        document.getElementById('login-userid').value = 'usr_' + Date.now() + '_fallback';
    }
